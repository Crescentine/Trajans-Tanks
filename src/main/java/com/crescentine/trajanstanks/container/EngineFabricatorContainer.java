...
public class EngineFabricatorContainer extends AbstractContainerMenu {
    private final EngineFabricatorBlockEntity blockEntity;
    private final Level level;
    private final ContainerData data;

    public EngineFabricatorContainer(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level.getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(2));
    }

    public EngineFabricatorContainer(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(TankModContainers.ENGINE_FABRICATOR_CONTAINER.get(), pContainerId);
        checkContainerSize(inv, 10);
        blockEntity = ((EngineFabricatorBlockEntity) entity);
